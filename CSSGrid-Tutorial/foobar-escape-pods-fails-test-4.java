...
// this works though ahhhhhhhhhh https://vitaminac.github.io/Google-Foobar-Escape-Pods/
public class Solution {
    ...
    public static class Edge {
        ...
        Edge(int value, int flow, int capacity, int reverse) {
            this.value = value;
            this.flow = flow;
    ...
    public static class Graph {
        ...
        void addEdge(int u, int v, int capacity) {
            ...
            adj.get(u).add(new Edge(v, 0, capacity, numV));
    ...
    public static int solution(int[] entrances, int[] exits, int[][] path) {
        ...
        */
        ...
        for (int entrance : entrances) {
            int sumFlowOut = 0;
            for (int out : path[entrance]) {
                sumFlowOut += out;
            }
            graph.addEdge(source, entrance, sumFlowOut);
        }
        for (int exit : exits) {
            int sumFlowIn = 0;
            for (int i = 0; i < path.length; i++) {
                sumFlowIn += path[i][exit];
            }
            graph.addEdge(exit, sink, sumFlowIn);
        }
        for (int i = 0; i < path.length; i++) {
